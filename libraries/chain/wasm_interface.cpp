...
#define READ_RECORD(FUNCNAME, OBJSTRUCT, SCOPE) \
   auto lambda = [&](apply_context* ctx, OBJSTRUCT::value_type::key_type* keys, char *data, uint32_t datalen) -> int32_t { \
      auto res = ctx->FUNCNAME<OBJSTRUCT, SCOPE>( Name(scope), Name(code), Name(table), keys, data, datalen); \
      if (res >= 0) res += OBJSTRUCT::value_type::number_of_keys*sizeof(OBJSTRUCT::value_type::key_type); \
      return res; \
   }; \
   return validate<decltype(lambda), OBJSTRUCT::value_type::key_type, OBJSTRUCT::value_type::number_of_keys>(valueptr, valuelen, lambda);

#define UPDATE_RECORD(FUNCNAME, OBJSTRUCT, DATASIZE) \
   auto lambda = [&](apply_context* ctx, OBJSTRUCT::value_type::key_type* keys, char *data, uint32_t datalen) -> int32_t { \
      return ctx->FUNCNAME<OBJSTRUCT::value_type>( Name(scope), Name(ctx->code.value), Name(table), keys, data, datalen); \
   }; \
   return validate<decltype(lambda), OBJSTRUCT::value_type::key_type, OBJSTRUCT::value_type::number_of_keys>(valueptr, DATASIZE, lambda);

#define DEFINE_RECORD_UPDATE_FUNCTIONS(OBJTYPE, RECSTRUCT) \
   DEFINE_INTRINSIC_FUNCTION4(env,store_##OBJTYPE,store_##OBJTYPE,i32,i64,scope,i64,table,i32,valueptr,i32,valuelen) { \
      UPDATE_RECORD(store_record, RECSTRUCT, valuelen); \
   } \
   DEFINE_INTRINSIC_FUNCTION4(env,update_##OBJTYPE,update_##OBJTYPE,i32,i64,scope,i64,table,i32,valueptr,i32,valuelen) { \
      UPDATE_RECORD(update_record, RECSTRUCT, valuelen); \
   } \
   DEFINE_INTRINSIC_FUNCTION3(env,remove_##OBJTYPE,remove_##OBJTYPE,i32,i64,scope,i64,table,i32,valueptr) { \
      UPDATE_RECORD(remove_record, RECSTRUCT, sizeof(typename RECSTRUCT::value_type::key_type)*RECSTRUCT::value_type::number_of_keys); \
   }

#define DEFINE_RECORD_READ_FUNCTIONS(OBJTYPE, INDEXNAME, RECSTRUCT, SCOPE) \
   DEFINE_INTRINSIC_FUNCTION5(env,load_##INDEXNAME##OBJTYPE,load_##INDEXNAME##OBJTYPE,i32,i64,scope,i64,code,i64,table,i32,valueptr,i32,valuelen) { \
      READ_RECORD(load_record, RECSTRUCT, SCOPE); \
   } \
   DEFINE_INTRINSIC_FUNCTION5(env,front_##INDEXNAME##OBJTYPE,front_##INDEXNAME##OBJTYPE,i32,i64,scope,i64,code,i64,table,i32,valueptr,i32,valuelen) { \
      READ_RECORD(front_record, RECSTRUCT, SCOPE); \
   } \
   DEFINE_INTRINSIC_FUNCTION5(env,back_##INDEXNAME##OBJTYPE,back_##INDEXNAME##OBJTYPE,i32,i64,scope,i64,code,i64,table,i32,valueptr,i32,valuelen) { \
      READ_RECORD(back_record, RECSTRUCT, SCOPE); \
   } \
   DEFINE_INTRINSIC_FUNCTION5(env,next_##INDEXNAME##OBJTYPE,next_##INDEXNAME##OBJTYPE,i32,i64,scope,i64,code,i64,table,i32,valueptr,i32,valuelen) { \
      READ_RECORD(next_record, RECSTRUCT, SCOPE); \
   } \
   DEFINE_INTRINSIC_FUNCTION5(env,previous_##INDEXNAME##OBJTYPE,previous_##INDEXNAME##OBJTYPE,i32,i64,scope,i64,code,i64,table,i32,valueptr,i32,valuelen) { \
      READ_RECORD(previous_record, RECSTRUCT, SCOPE); \
   } \
   DEFINE_INTRINSIC_FUNCTION5(env,lower_bound_##INDEXNAME##OBJTYPE,lower_bound_##INDEXNAME##OBJTYPE,i32,i64,scope,i64,code,i64,table,i32,valueptr,i32,valuelen) { \
      READ_RECORD(lower_bound_record, RECSTRUCT, SCOPE); \
   } \
   DEFINE_INTRINSIC_FUNCTION5(env,upper_bound_##INDEXNAME##OBJTYPE,upper_bound_##INDEXNAME##OBJTYPE,i32,i64,scope,i64,code,i64,table,i32,valueptr,i32,valuelen) { \
      READ_RECORD(upper_bound_record, RECSTRUCT, SCOPE); \
   }
